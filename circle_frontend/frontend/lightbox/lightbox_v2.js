...
function _clearPendingClose() {
  if (window._lbCloseTimer) {
    clearTimeout(window._lbCloseTimer);
    window._lbCloseTimer = null;
  }
...
function openProfilePicLightbox(picUrl, userName) {
  ...
  if (lbCommPanel) lbCommPanel.style.display = "none";
  if (lbRepPanel) lbRepPanel.style.display = "none";
  ...
  if (lb) lb.dataset.profilePicMode = "1";
...
async function lbSubmitComment() {
  ...
  try {
    ...
    if (!newComment) throw new Error("Invalid response from server");
...
function _lbShowItem() {
  ...
  if (item.type === "video") {
    ...
    if (lbVid) {
      ...
      requestAnimationFrame(() => {
        if (lbVid) lbVid.style.opacity = "1";
        lbVid.play().catch(() => {});
      });
    }
  } else {
...
function openLightbox(imgEl) {
  ...
  if (lbActsReset) lbActsReset.style.display = "none";
...
function openVideoLightbox(wrapEl) {
  ...
  if (_lbIndex < 0) {
    const meta = {
      name: wrapEl.dataset.lbName || null,
      picture: wrapEl.dataset.lbPicture || null,
      userId: wrapEl.dataset.lbUserId ? parseInt(wrapEl.dataset.lbUserId, 10) : null,
      postId: wrapEl.dataset.lbPostId ? parseInt(wrapEl.dataset.lbPostId, 10) : null,
      caption: wrapEl.dataset.lbCaption || null,
    };
    _lbItems = [{ type: "video", src: videoSrc, meta }];
    _lbIndex = 0;
  }
  _lbScale = 1;
  _lbTranslateX = 0;
...
function lbGoTo(newIdx) {
  ...
  if (fromVideo && lbVid) lbVid.pause();
...
function closeLightbox() {
  if (window._lbCloseTimer) clearTimeout(window._lbCloseTimer);
  ...
  if (lb && lb.dataset.profilePicMode) {
  ...
  window._lbCloseTimer = setTimeout(() => {
    if (lb) {
...
function lbWheel(e) {
  ...
  if (Math.abs(navDelta) > Math.abs(zoomDelta) || isUD) {
    if (!lbWheel._lastNavTime || Date.now() - lbWheel._lastNavTime > 200) {
      lbGoTo(_lbIndex + (navDelta > 0 ? 1 : -1));
      lbWheel._lastNavTime = Date.now();
    }
    return;
  }
